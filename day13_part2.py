...
def move_cart(index):
    ...
    # check if the temp_track_grid next track position has a cart ... colision!!
        ...
        # get the two carts that have collided, and remove them from the list
        del carts[index]

        del_indexes = list()
        for i in range(len(carts)):
            if carts[i]['pos_x'] == next_pos_x and carts[i]['pos_y'] == next_pos_y:
...
while len(carts) > 1:
    for i in range(len(carts)):
        if move_cart(i) == False:
            print('collision ... num carts remaining: {}'.format(len(carts)))
            if len(carts) == 1:
                # from pdb import set_trace; set_trace()
                print(carts)
                move_cart(i)
